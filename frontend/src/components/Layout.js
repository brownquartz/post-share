...
function NotificationsPanel({ items, onClose, onMarkRead, onDelete, onDeleteAll, hasMore, onLoadMore }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-5 py-4 border-b border-[#352d55]">
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-100 hover:bg-[#241d3d] transition-colors">
      ...
      <div className="flex-1 overflow-y-auto py-2">
        {filtered.length === 0 ? (
          ...
          <ul className="divide-y divide-[#352d55]">
            {filtered.map(item => (
              <li key={item.id} className={`px-5 py-3 flex gap-2 ${!item.isRead ? 'bg-purple-900/20' : ''}`}>
        ...
        {hasMore && (
          <div className="px-5 py-3 border-t border-[#352d55]">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-6">
        ...
        <div>
          ...
          <form onSubmit={handleSendRequest} className="flex gap-2">
            <input
              className="flex-1 bg-[#241d3d] border border-[#4a3d72] rounded-lg px-3 py-2 text-sm text-gray-100 placeholder-gray-500 focus:outline-none focus:border-brand"
              placeholder="ユーザー名"
              value={username}
        ...
        {requests.length > 0 && (
          <div>
            ...
            <ul className="space-y-2">
              {requests.map(r => (
                <li key={r.id} className="flex items-center justify-between gap-2 bg-[#241d3d] rounded-lg px-3 py-2">
                  ...
                  <div className="flex gap-1.5">
                    ...
                    <button onClick={() => handleReject(r.id)} className="px-2 py-1 bg-[#352d55] hover:bg-[#4a3d72] text-gray-300 text-xs rounded transition-colors">拒否</button>
                  </div>
                </li>
              ))}
            </ul>
        ...
        <div>
          ...
          {friends.length === 0 ? (
            ...
            <ul className="space-y-2">
              {friends.map(f => (
                <li key={f.id} className="flex items-center justify-between gap-2 bg-[#241d3d] rounded-lg px-3 py-2">
...
function Drawer({ isOpen, onClose, user, authReady, signOut, toggleTheme, isDark }) {
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 left-0 z-40 h-full w-64 bg-[#18142e] border-r border-[#352d55]
                      flex flex-col transition-transform duration-300 ease-in-out
                      ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#352d55]">
          <span className="font-bold text-brand text-lg tracking-tight">Post Share</span>
          <button type="button" onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-100 hover:bg-[#241d3d] transition-colors" aria-label="Close menu">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
              ...
              <Link key={href} href={href} onClick={onClose}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors
                  ${active ? 'bg-brand/15 text-brand'
                    : admin ? 'text-yellow-400 hover:bg-[#241d3d] hover:text-yellow-300'
                    : 'text-gray-300 hover:bg-[#241d3d] hover:text-gray-100'}`}
              >
        ...
        <div className="border-t border-[#352d55] px-3 py-4 space-y-2">
          <button type="button" onClick={() => { toggleTheme(); }}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gray-300 hover:bg-[#241d3d] hover:text-gray-100 transition-colors">
            <span>{isDark ? '☀️' : '🌙'}</span>
          ...
          {!authReady ? null : user ? (
            ...
            <>
              <Link href="/auth/login" onClick={onClose} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gray-300 hover:bg-[#241d3d] hover:text-gray-100 transition-colors">ログイン</Link>
...
function RightPanel({ type, onClose, notifItems, friends, friendRequests, onMarkRead, onReloadFriends, onDeleteNotif, onDeleteAllNotifs, notifHasMore, onLoadMoreNotifs }) {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 z-40 h-full w-full sm:w-80 bg-[#18142e] border-l border-[#352d55] flex flex-col">
        {type === 'notifications' && (
    ...
    <div className="page-bg flex flex-col">
      <nav className="header-bar flex items-center justify-between px-5 py-3 sticky top-0 z-10">
        ...
        <button type="button" onClick={() => setDrawerOpen(true)}
          className="w-9 h-9 flex items-center justify-center rounded-lg text-gray-300 hover:text-gray-100 hover:bg-[#241d3d] transition-colors cursor-pointer" aria-label="Open menu">
          <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="flex items-center gap-1">
          ...
          {user && (
            <button type="button" onClick={() => openPanel('notifications')}
              className="relative w-9 h-9 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-200 hover:bg-[#241d3d] transition-colors cursor-pointer"
              aria-label="お知らせ">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {/* テーマ切替 */}
          <button type="button" onClick={toggleTheme}
            className="w-9 h-9 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-200 hover:bg-[#241d3d] transition-colors cursor-pointer"
      ...
      <footer className="border-t border-gray-200 dark:border-[#352d55] text-center py-3 text-xs text-gray-400 dark:text-gray-600">
        © 2025 Post Share
      </footer>
    </div>
  );
